...
func StartWorkspace(workspace *types.Workspace, target *provider.ProviderTarget) error {
...
func StartProject(project *types.Project, target *provider.ProviderTarget) error {
	targetProvider, err := manager.GetProvider(target.ProviderInfo.Name)
	if err != nil {
		return err
	}
