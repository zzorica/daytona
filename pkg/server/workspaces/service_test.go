...
func TestWorkspaceService(t *testing.T) {
	...
	t.Run("CreateWorkspace", func(t *testing.T) {
		...
		projectConfig := config.ProjectConfig{
			Name: createWorkspaceRequest.Projects[0].NewProjectConfig.Name,
			Repository: &gitprovider.GitRepository{
				Id:   createWorkspaceRequest.Projects[0].NewProjectConfig.Source.Repository.Id,
				Url:  createWorkspaceRequest.Projects[0].NewProjectConfig.Source.Repository.Url,
				Name: createWorkspaceRequest.Projects[0].NewProjectConfig.Source.Repository.Name,
			},
			Image: defaultProjectImage,
			User:  defaultProjectUser,
		}

		projectConfigService.On("ToProjectConfig", mock.Anything).Return(&projectConfig)
	...
	t.Run("StartProject", func(t *testing.T) {
		...
		err := service.StartProject(context.TODO(), createWorkspaceRequest.Id, createWorkspaceRequest.Projects[0].NewProjectConfig.Name)
	...
	t.Run("StopProject", func(t *testing.T) {
		...
		err := service.StopProject(context.TODO(), createWorkspaceRequest.Id, createWorkspaceRequest.Projects[0].NewProjectConfig.Name)
