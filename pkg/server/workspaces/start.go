...
func (s *WorkspaceService) StartWorkspace(workspaceId string) error {
	...
	workspaceLogger := s.loggerFactory.CreateWorkspaceLogger(w.Id, logger.LogSourceServer)
...
func (s *WorkspaceService) StartProject(workspaceId, projectName string) error {
	...
	projectLogger := s.loggerFactory.CreateProjectLogger(w.Id, project.Name, logger.LogSourceServer)
...
func (s *WorkspaceService) startWorkspace(workspace *workspace.Workspace, target *provider.ProviderTarget, wsLogWriter io.Writer) error {
	...
	for _, project := range workspace.Projects {
		projectLogger := s.loggerFactory.CreateProjectLogger(workspace.Id, project.Name, logger.LogSourceServer)
		defer projectLogger.Close()
