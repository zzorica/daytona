...
func (bl *buildLogger) Write(p []byte) (n int, err error) {
	if bl.logFile == nil {
		filePath := filepath.Join(bl.logsDir, bl.projectName, "log")
...
func (bl *buildLogger) Cleanup() error {
	projectLogsDir := filepath.Join(bl.logsDir, bl.projectName)
...
func (l *loggerFactoryImpl) CreateBuildLogger(projectName, hash string, source LogSource) Logger {
	logger := logrus.New()
...
func (l *loggerFactoryImpl) CreateBuildLogReader(projectName, hash string) (io.Reader, error) {
	filePath := filepath.Join(l.logsDir, projectName, "log")
